...
router.post('/', async (req, res) => {
    try {
        ...
        var findUser = await User.findById(rUserId, { isActive: 1, taskDate: 1, taskdata: 1, scratchdata: 1 }).lean();
        ...
        if (DiffDay == 0 && findActive) {
            var totalScratch = findUser.scratchdata;
            var filterTaskByApplication = totalScratch.filter(data => data.appId == rAppid);
            var data = {
                code: 1,
                msg: MSG.common[0],
                isActive: findActive,
                scratchdata: filterTaskByApplication
            }
            ...
            var findApplication = await Application.find({ isActive: true, todayassigntask: 1, todayassignclick: 1, taskclickpercentage: 1 }, { uan: 1 }).sort({totalassignclick: 1}).lean();;
            ...
            var updateTaskByUser = await User.findByIdAndUpdate(rUserId, { $set: { taskdata: taskArray, taskDate: CurrentDate } }, {
                "fields": { "scratchdata": 1, "_id": 0 },
                "new": true
            });
            var totalTask = updateTaskByUser.scratchdata;
