...
router.post('/', async (req, res) => {
    try {
        ...
        var findUser = await User.findById(rUserId).select({
            isActive: 1, mobileNo: 1, deviceId: 1, uniqueId: 1, isFirstWithdraw: 1, name: 1, minwithcoin: 1, wallet: 1,
            coin: 1, amount: 1, convertCoin: 1, convertAmount: 1, withdrawDate: 1, referCoin: 1, reference: 1, getreferCoin: 1
        });
        var findWallet = findUser.wallet.map(data => data.uan == rUan);
        var findIsFirstWithdraw = findUser.isFirstWithdraw; // Set true after one withdraw
        var checkbalance = findWallet[0].coin - parseInt(rCoin); // balance must be grater than zero
